...
const ImageCropper = (_ref, ref) => {
  ...
  const initiateCropper = async (newImagePath, newWidth, newHeight) => {
    let imagePath = imageUrl ? imageUrl : await (0, _.resolveImagePath)(newImagePath || source.uri);
    if (!imageUrl) setImageUrl(imagePath);
    // else imagePath = imageUrl;
    _reactNative.Image.getSize(`file://` + imagePath.replace('file://', ''), (width, height) => {
      setWidth(newWidth || width);
      setHeight(newHeight || height);
      const newViewHeight = WINDOW_WIDTH * ((newHeight || height) / (newWidth || width));
